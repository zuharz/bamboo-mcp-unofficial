...
export class BambooClient {
    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            this.logger.error('BambooHR API request failed after all retries:', method, endpoint, error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    /**
     ...
     */
    async makeHttpRequestWithRetry(endpoint, options) {
        let lastError;
            ...
            try {
                ...
                if (response.status === 429) {
                    ...
                    if (attempt < this.config.maxRetryAttempts) {
                        this.logger.warn(`Rate limit hit (429) for ${endpoint}, attempt ${attempt + 1}/${this.config.maxRetryAttempts + 1}, waiting ${retryAfter}ms`);
                ...
                if (response.status >= 500 && response.status < 600) {
                    if (attempt < this.config.maxRetryAttempts) {
                        ...
                        this.logger.warn(`Server error (${response.status}) for ${endpoint}, attempt ${attempt + 1}/${this.config.maxRetryAttempts + 1}, waiting ${delay}ms`);
                ...
                if (this.isRetryableNetworkError(lastError) &&
                    ...
                    const delay = this.calculateExponentialBackoffDelay(attempt);
                    this.logger.warn(`Network error for ${endpoint}, attempt ${attempt + 1}/${this.config.maxRetryAttempts + 1}, waiting ${delay}ms: ${lastError.message}`);
                    await this.sleep(delay);
                    continue; // Retry the request
                }
    ...
    async makeHttpRequest(endpoint, options) {
        ...
        try {
            ...
            if (networkError instanceof Error && networkError.name === 'AbortError') {
                const timeoutSeconds = this.config.requestTimeoutMs / 1000;
                throw new Error(`Request to BambooHR API timed out after ${timeoutSeconds} seconds: ${endpoint}`);
            }
    ...
    async parseResponse(response, endpoint) {
        ...
        try {
            ...
            this.logger.error('Failed to parse BambooHR API response:', endpoint, 'Response length:', responseText.length, 'Response preview:', responseText.substring(0, 200), 'Parse error:', parseError instanceof Error ? parseError.message : parseError);
            // Check if response looks like HTML (common error page)
            if (responseText.trim().startsWith('<')) {
                throw new Error(`BambooHR returned HTML instead of JSON. This usually indicates an authentication or server error. Endpoint: ${endpoint}`);
            }
            // Check if response is partial JSON
            if (responseText.trim().startsWith('{') ||
                responseText.trim().startsWith('[')) {
                throw new Error(`Incomplete JSON response from BambooHR API. Response was truncated or corrupted. Endpoint: ${endpoint}`);
            }
        ...
        try {
            ...
            if (errorText) {
                ...
                try {
                    ...
                    const truncatedText = errorText.length > 500
    ...
    getRetryAfterDelay(response) {
        ...
        return Math.min(this.config.retryBaseDelayMs * 2, this.config.retryMaxDelayMs);
    }
    /**
 ...
 */
export function createBambooClient(config, logger) {
    return new BambooClient(config, logger);
}
/**
 * Create a BambooClient for testing with sensible defaults
 */
export function createTestBambooClient(overrides = {}, logger) {
    const config = {
        apiKey: 'test-api-key',
