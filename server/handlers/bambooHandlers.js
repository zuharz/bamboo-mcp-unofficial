...
export async function handleWhosOut(args, _context = {}) {
    try {
        ...
        const endDate = end;
        const calendar = await bambooClient.get(`/time_off/whos_out?start=${startDate}&end=${endDate}`);
...
export async function handleTimeOffRequests(args, _context = {}) {
    try {
        ...
        const text = formatters.formatTimeOffRequests(requests || [], start_date, end_date);
...
export async function handleDiscoverDatasets(_args, context = {}) {
    try {
        // Support both direct progressToken and _meta.progressToken
        const progressToken = context.progressToken || context._meta?.progressToken;
        // Create progress tracker
        ...
        const text = formatters.formatDatasetsList(datasets.datasets || []);
        return { content: [{ type: 'text', text }] };
    }
    catch (error) {
...
export async function handleDiscoverFields(args, _context = {}) {
    try {
        ...
        const text = formatters.formatDatasetFields(fields.fields || [], dataset_id);
        return { content: [{ type: 'text', text }] };
    }
    catch (error) {
        return {
...
export async function handleWorkforceAnalytics(args, context = {}) {
    try {
...
- Check if the dataset has any data at all`,
        ...
        const text = formatters.formatWorkforceAnalytics(records, dataset_id, requestPayload.fields);
        ...
        logger.error('Workforce analytics failed:', error.message);
        return {
            content: [
                {
                    type: 'text',
...
Available properties: ${reports && typeof reports === 'object'
...
- Average Department Size: ${Math.round(totalEmployees / departments.length)}`;
        ...
        return formatters.formatApiErrorResponse(error, '/employees/directory', undefined);
    }
}
