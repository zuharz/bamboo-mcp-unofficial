...
let bambooClient;
let logger;
export function initializeDatasetHandlers(dependencies) {
...
export async function handleDiscoverDatasets(_args, context = {}) {
    try {
        ...
        const text = formatters.formatDatasetsList(datasetsResponse.datasets || []);
        return { content: [{ type: 'text', text }] };
    }
    catch (error) {
        return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'dataset discovery', 'bamboo_discover_datasets', {
            endpoint: '/datasets',
        });
    }
}
export async function handleDiscoverFields(args, _context = {}) {
    try {
        ...
        const text = formatters.formatDatasetFields(fieldsResponse.fields || [], dataset_id);
        return { content: [{ type: 'text', text }] };
    }
    catch (error) {
        return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'field discovery', 'bamboo_discover_fields', {
