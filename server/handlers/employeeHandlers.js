...
export async function handleGetEmployeePhoto(args, _context = {}) {
    ...
    const size = args.size || 'small'; // Default to small - compression will handle byte reduction
    try {
        if (!employee_id) {
            return {
                content: [
        ...
        if (return_base64) {
            ...
            const { formatBytes, validateImageBuffer, createDataUri } = await import('../utils/imageUtils.js');
            // This is where the magic happens - fetch the actual image bytes with size control
            const imageBuffer = await bambooClient.getBinary(`/employees/${employee_id}/photo/${size}`);
