...
Available properties: ${reports && typeof reports === 'object'
                ...
                return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'custom reports listing', 'bamboo_run_custom_report', {
...
2. Then use the report ID from the list to run a specific report`,
        ...
        try {
            ...
            try {
                ...
                return handleBambooError(apiError instanceof Error ? apiError : new Error(String(apiError)), 'custom report execution', 'bamboo_run_custom_report', {
...
TIP: Try again in a few moments or use \`{"list_reports": true}\` to verify the report exists.`,
            ...
            return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'custom report processing', 'bamboo_run_custom_report', {
                parameters: { report_id, format },
        ...
        return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'custom report operation', 'bamboo_run_custom_report', {
            parameters: {
