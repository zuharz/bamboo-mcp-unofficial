...
export async function handleWhosOut(args, _context = {}) {
    try {
        ...
        return handleBambooError(error instanceof Error ? error : new Error(String(error)), "who's out calendar", 'bamboo_whos_out', {
...
export async function handleTimeOffRequests(args, _context = {}) {
    try {
        ...
        const text = formatters.formatTimeOffRequests(requestsArray, start_date, end_date);
        return { content: [{ type: 'text', text }] };
    }
    catch (error) {
        return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'time-off requests retrieval', 'bamboo_time_off_requests', {
            parameters: {
