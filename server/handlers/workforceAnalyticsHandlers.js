...
let bambooClient;
let logger;
export function initializeWorkforceAnalyticsHandlers(dependencies) {
    bambooClient = dependencies.bambooClient;
    logger = dependencies.logger;
    mcpLogger.analytics('info', 'Workforce analytics handlers initialized successfully');
    return true;
}
...
\`{"filters": [{"field": "status", "operator": "equal", "value": "Active"}]}\``,
        ...
        const requestPayload = {
            fields: fieldsArray.filter((f) => typeof f === 'string' && f.trim().length > 0),
        };
...
**Valid filters**: ${validFilters.length}/${filtersArray.length}`,
        ...
        try {
            ...
            return handleBambooError(networkError instanceof Error
                ? networkError
                : new Error(String(networkError)), 'workforce analytics query', 'bamboo_workforce_analytics', {
...
- Check if the dataset has any data at all`,
        ...
        return handleBambooError(error instanceof Error ? error : new Error(String(error)), 'workforce analytics execution', 'bamboo_workforce_analytics', {
            parameters: {
