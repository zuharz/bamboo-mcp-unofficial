...
import { formatApiErrorResponse, formatCustomReportResults, formatCustomReportsList, formatDatasetFields, formatDatasetsList, formatEmployeeList, formatErrorResponse, formatNetworkError, formatTimeOffRequests, formatWhosOutList, formatWorkforceAnalytics, } from './formatters.js';
const logger = {
    debug: (msg, ...args) => {
...
async function bambooGet(endpoint) {
    return bambooClient.get(endpoint);
}
async function bambooPost(endpoint, body) {
...
server.tool('bamboo_find_employee', 'Find employee by name, email, or ID with support for partial name matches', {
    ...
    try {
        ...
        const employees = (await bambooGet('/employees/directory?fields=id,firstName,lastName,workEmail,jobTitle,department'));
...
server.tool('bamboo_whos_out', 'See who is out on leave today or in date range. Defaults to today if no dates provided.', {
    ...
    try {
        ...
        const endDate = end;
        const calendar = (await bambooGet(`/time_off/whos_out?start=${startDate}&end=${endDate}`));
...
server.tool('bamboo_team_info', 'Get team/department roster with employee details including job titles and contact info', {
    ...
    try {
        ...
        const employees = (await bambooGet('/employees/directory?fields=firstName,lastName,workEmail,jobTitle,department'));
...
server.tool('bamboo_time_off_requests', 'Get time-off requests for date range', {
    ...
    try {
        ...
        const requests = (await bambooGet(endpoint));
...
server.tool('bamboo_discover_datasets', 'Discover what datasets are available in BambooHR for analytics', async () => {
    try {
        const datasets = (await bambooGet('/datasets'));
...
server.tool('bamboo_discover_fields', 'Discover what fields are available in a specific dataset for use in workforce analytics', {
    ...
    try {
        ...
        const fields = (await bambooGet(`/datasets/${dataset_id}/fields`));
...
server.tool('bamboo_get_employee_photo', 'Get the profile photo for a specific employee by their ID', {
    employee_id: z
        .string()
...
server.tool('bamboo_list_departments', 'Get a list of all departments in the company', async () => {
    try {
        // Use employee directory endpoint instead of analytics to avoid groupBy issues
        const employees = (await bambooGet('/employees/directory?fields=department'));
