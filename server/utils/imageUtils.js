 ...
 */
export function compressImageBuffer(buffer, quality = 0.7) {
    ...
    if (getImageMimeType(buffer) === 'image/jpeg') {
        ...
        if (targetSize < buffer.length) {
            ...
            for (let i = 0; i < Math.min(20, buffer.length) && compressedIndex < compressed.length; i++) {
                compressed[compressedIndex++] = buffer[i];
            }
            // Sample the rest of the data
            for (let i = 20; i < buffer.length && compressedIndex < compressed.length; i += step) {
                compressed[compressedIndex++] = buffer[i];
            }
 ...
 */
export function createDataUri(buffer, compress = false) {
