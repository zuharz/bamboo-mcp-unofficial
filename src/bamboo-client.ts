...
export class BambooClient {
  ...
  async getBinary(endpoint: string): Promise<Buffer> {
    ...
    if (endpoint.includes('/photo')) {
      ...
      const subdomainMatch = this.config.baseUrl.match(
        /gateway\.php\/([^/]+)\/v1/
      );
      if (!subdomainMatch) {
        throw new Error(
          'Unable to extract subdomain from baseUrl for photo request'
        );
      }
    ...
    this.logger.debug(
      'Making BambooHR binary API request:',
      endpoint,
      'URL:',
      url
    );
    ...
    try {
      ...
      this.logger.info(
        'BambooHR binary API request completed successfully:',
        endpoint,
        'status:',
        response.status,
        'size:',
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        throw new Error(
          `Binary request to BambooHR API timed out after ${timeoutSeconds} seconds: ${endpoint}`
        );
      }
      ...
      throw new Error(
        `Network error during binary request to BambooHR API: ${error instanceof Error ? error.message : 'Unknown network error'}`
      );
    }
  }
