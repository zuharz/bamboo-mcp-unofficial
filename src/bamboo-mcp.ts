...
const logger: SimpleLogger = {
  ...
  fatal: (msg: string, ...args: unknown[]) => console.error('[FATAL]', msg, ...args),
  child: () => logger
};
...
server.tool('bamboo_find_employee', 'Find employee by name, email, or ID with support for partial name matches', {
  query: z.string().describe('Employee name, email, or ID to search for. Examples: "John Smith", "john.smith@company.com", "123"')
}, async (args: { query: string }) => {
  try {
    const query = args.query;
    
    logger.debug('Employee search initiated, query length:', query?.length || 0);
    
    if (!query || typeof query !== 'string' || query.trim() === '') {
      logger.warn('Employee search failed - missing query parameter');
      return { content: [{ type: 'text', text: 'Missing required parameter: query. Provide employee name, email, or ID to search for.' }] };
    }
    
    const queryLower = query.toLowerCase();

    const employees = await bambooGet('/employees/directory?fields=id,firstName,lastName,workEmail,jobTitle,department') as BambooEmployeeDirectory;
    ...
    if (!found) {
      ...
      return { content: [{ type: 'text', text: `No employee found matching "${query}"` }] };
    }
...
Employee ID: ${found.id}`;
    ...
    logger.error('Employee search failed with error:', error instanceof Error ? error.message : error);
    return formatErrorResponse(error, 'Employee search failed');
  }
});

// Tool 2: Who's Out
server.tool('bamboo_whos_out', 'See who is out on leave today or in date range. Defaults to today if no dates provided.', {
  start_date: z.string().optional().describe('Start date in YYYY-MM-DD format (optional, defaults to today). Example: "2024-01-15"'),
  end_date: z.string().optional().describe('End date in YYYY-MM-DD format (optional, defaults to start_date). Example: "2024-01-20"')
}, async (args: { start_date?: string | undefined; end_date?: string | undefined }) => {
  try {
    ...
    const calendar = await bambooGet(`/time_off/whos_out?start=${startDate}&end=${endDate}`) as { calendar?: BambooWhosOutEntry[] };
...
// Tool 3: Team Info
server.tool('bamboo_team_info', 'Get team/department roster with employee details including job titles and contact info', {
  department: z.string().describe('Department name to get roster for. Supports partial matching. Examples: "Engineering", "Product", "QA", "Sales"')
}, async (args: { department: string }) => {
  try {
    const department = args.department;
    if (!department || typeof department !== 'string' || department.trim() === '') {
      return { content: [{ type: 'text', text: 'Missing required parameter: department. Provide department name to get team roster for.' }] };
    }

    const employees = await bambooGet('/employees/directory?fields=firstName,lastName,workEmail,jobTitle,department') as BambooEmployeeDirectory;
    
    const teamMembers = employees.employees?.filter((emp: BambooEmployee) => 
      emp.department?.toLowerCase().includes(department.toLowerCase())
    ) || [];
    
    if (!teamMembers.length) {
      return { content: [{ type: 'text', text: `No employees found in department "${department}"` }] };
    }
...
server.tool('bamboo_time_off_requests', 'Get time-off requests for date range', {
  ...
  status: z.string().optional().describe('Filter by request status (approved, denied, pending, all). Defaults to all')
}, async (args: { start_date: string; end_date: string; status?: string | undefined }) => {
  try {
    ...
    if (!start_date || !end_date || typeof start_date !== 'string' || typeof end_date !== 'string') {
      return { content: [{ type: 'text', text: 'Missing required parameters: start_date and end_date (YYYY-MM-DD format). Optional status filter: approved, denied, pending, all.' }] };
    }
...
server.tool('bamboo_discover_datasets', 'Discover what datasets are available in BambooHR for analytics', async () => {
  try {
    const datasets = await bambooGet('/datasets') as BambooDatasetsResponse;
    
    if (!datasets?.datasets?.length) {
      return { content: [{ type: 'text', text: 'No datasets available or API not accessible' }] };
    }
...
server.tool('bamboo_discover_fields', 'Discover what fields are available in a specific dataset for use in workforce analytics', {
  dataset_id: z.string().describe('Dataset ID to explore (use bamboo_discover_datasets first to get IDs). Examples: "employee", "time_off", "performance"')
}, async (args: { dataset_id: string }) => {
  try {
    const dataset_id = args.dataset_id;
    if (!dataset_id || typeof dataset_id !== 'string' || dataset_id.trim() === '') {
      return { content: [{ type: 'text', text: 'Missing required parameter: dataset_id. Use bamboo_discover_datasets first to get available dataset IDs.' }] };
    }
    
    const fields = await bambooGet(`/datasets/${dataset_id}/fields`) as BambooFieldsResponse;
    
    if (!fields?.fields?.length) {
      return { content: [{ type: 'text', text: `No fields found for dataset: ${dataset_id}` }] };
    }
    ...
    return { content: [{ type: 'text', text: `ERROR: Error discovering fields: ${error instanceof Error ? error.message : 'Unknown error'}` }] };
  }
});

// Tool 7: Workforce Analytics (Step 3 - Enhanced with robust edge case handling)
server.tool('bamboo_workforce_analytics', 'Get workforce analytics data from BambooHR datasets - use discovery tools first to find correct dataset and field names', {
  dataset_id: z.string().describe('Dataset ID (use bamboo_discover_datasets to find available datasets)'),
  fields: z.array(z.string()).describe('Array of field names to retrieve (use bamboo_discover_fields to find available fields)'),
  filters: z.array(z.object({
    field: z.string(),
    operator: z.string(),
    value: z.any()
  })).optional().describe('Optional filters to apply to the data'),
  group_by: z.string().optional().describe('Optional field name to group results by')
}, async (args: WorkforceAnalyticsArgs) => {
  try {
    const { dataset_id, fields, filters, group_by } = args;
    
    // Enhanced input validation
    if (!dataset_id || !fields || !Array.isArray(fields) || fields.length === 0) {
      return { content: [{ type: 'text', text: `Discovery required first. To use workforce analytics:
...
Example: {"dataset_id": "employee", "fields": ["department", "status"]}` }] };
    ...
    if (filters && !Array.isArray(filters)) {
      return { content: [{ type: 'text', text: `ERROR: **Invalid Filters Format**
...
\`{"filters": [{"field": "status", "operator": "equal", "value": "Active"}]}\`` }] };
    ...
    if (requestPayload.fields.length === 0) {
      return { content: [{ type: 'text', text: `ERROR: **Invalid Fields**
...
TIP: Use \`bamboo_discover_fields\` to see available field names for the dataset.` }] };
    ...
    if (group_by && typeof group_by === 'string' && group_by.trim().length > 0) {
    ...
    if (filters && filters.length > 0) {
      ...
      if (validFilters.length !== filters.length) {
        return { content: [{ type: 'text', text: `ERROR: **Invalid Filter Structure**
...
Raw response: ${JSON.stringify(data)}` }] };
    ...
    if (Array.isArray(data)) {
      ...
      if (dataObj.data && Array.isArray(dataObj.data)) {
        ...
        return { content: [{ type: 'text', text: `ERROR: **Unexpected Response Structure**
...
Raw response: ${JSON.stringify(data, null, 2)}` }] };
    ...
    if (records.length === 0) {
      return { content: [{ type: 'text', text: `ANALYTICS: **No Records Found**
...
- Check if the dataset has any data at all` }] };
    ...
    const text = formatWorkforceAnalytics(records, dataset_id, requestPayload.fields);
    return { content: [{ type: 'text', text }] };
    
  } catch (error) {
    return { content: [{ type: 'text', text: `ERROR: **Unexpected Error**
...
server.tool('bamboo_run_custom_report', 'List available custom reports or run a specific report by ID with multiple output formats', {
  list_reports: z.boolean().optional().describe('Set to true to list all available custom reports. Example: {"list_reports": true}'),
  report_id: z.string().optional().describe('ID of specific report to run (get from list_reports first). Example: {"report_id": "123"}'),
  format: z.enum(['json', 'csv', 'pdf']).optional().describe('Output format for the report (defaults to json). Example: {"report_id": "123", "format": "json"}')
  ...
  try {
    ...
    try {
      ...
      return { content: [{ type: 'text', text: `ERROR: **Unexpected Error Running Report**\n\nReport ID: ${report_id}\nError: ${error instanceof Error ? error.message : 'Unknown error'}\n\nTIP: **Troubleshooting:**\n- Verify the report ID exists using \`{"list_reports": true}\`\n- Check your API key permissions\n- Try a different output format (json, csv, pdf)\n- Contact your BambooHR administrator if the issue persists\n\n**Stack trace** (if available): ${error instanceof Error && error.stack ? error.stack : 'Not available'}` }] };
    }
  } catch (error) {
    return { content: [{ type: 'text', text: `ERROR: General error: ${error instanceof Error ? error.message : 'Unknown error'}` }] };
  }
});
