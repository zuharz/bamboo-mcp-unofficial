...
// Minimal global declarations to satisfy type-checking without @types/node
...
// ----------------------------------------------------------------------------
// Simple stderr logger compatible with existing handlers
// ----------------------------------------------------------------------------
...
s.tool(
  ...
  'Get the profile photo for a specific employee by their ID. Returns actual image data as base64 for display or URL for external use.',
  {
    employee_id: z
      .string()
      .describe(
    ...
    return_base64: z
      ...
      .default(true)
      .describe(
        'Whether to return base64 image data for display (true) or just the authenticated URL (false). Defaults to true.'
      ),
    size: z
      ...
      .default('small')
      .describe(
        'Photo size to retrieve. Smaller sizes help avoid message limits.'
      ),
  },
  async (args: Record<string, unknown>) => {
    try {
...
process.on('uncaughtException', (error: unknown) => {
  logger.fatal('Uncaught exception occurred:', (error as Error).message);
  process.exit(1);
});
