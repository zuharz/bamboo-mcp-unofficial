...
interface BambooErrorResult {
  userMessage: string;
  errorType: BambooErrorType;
  isRetryable: boolean;
...
class BambooErrorHandler {
  private readonly httpStatusMessages: Record<number, BambooErrorType> = {
   ...
   */
  categorizeError(error: Error): BambooErrorType {
   ...
   */
  generateUserMessage(errorType: BambooErrorType, operation: string): string {
    const messages: Record<BambooErrorType, string> = {
   ...
   */
  getTroubleshootingSteps(errorType: BambooErrorType): string[] {
    const steps: Record<BambooErrorType, string[]> = {
   ...
   */
  isRetryable(errorType: BambooErrorType): boolean {
    return [
      BambooErrorType.RATE_LIMIT,
      BambooErrorType.NETWORK_ERROR,
      BambooErrorType.TIMEOUT_ERROR,
   ...
   */
  getRetryDelay(errorType: BambooErrorType, attempt: number = 1): number {
    const baseDelays: Record<BambooErrorType, number> = {
   ...
   */
  handleError(error: Error, context: ErrorContext): MCPToolResponse {
...
${troubleshootingText}${retryNote}`,
   ...
   */
  analyzeError(error: Error, context: ErrorContext): BambooErrorResult {
    const errorType = this.categorizeError(error);
    const userMessage = this.generateUserMessage(errorType, context.operation);
    const troubleshooting = this.getTroubleshootingSteps(errorType);
    const isRetryable = this.isRetryable(errorType);
 ...
 */
export function analyzeBambooError(
  error: Error,
  operation: string,
  toolName: string,
  additionalContext?: Partial<ErrorContext>
...
export type { ErrorContext, BambooErrorResult };
